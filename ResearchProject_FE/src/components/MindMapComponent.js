...
const MindMapComponent = () => {
  ...
  const generateMindMap = async () => {
    ...
    try {
      ...
      console.log('Received mind map data:', data.mindmap);

      // Create URL with mind map data as a parameter
      const mindMapDataParam = encodeURIComponent(JSON.stringify(data.mindmap));
      const mindMapUrl = chrome.runtime.getURL('mindmap.html') + `?data=${mindMapDataParam}`;
      console.log('Created mind map URL:', mindMapUrl);

      // Open the mind map in a new tab
      if (chrome.tabs) {
        chrome.tabs.create({ url: mindMapUrl });
      } else {
